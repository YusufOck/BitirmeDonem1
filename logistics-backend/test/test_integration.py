...
Senaryo: 3 durak, mock ML delay — cost matrix formülünü doğrular.
  ...
  Baseline [3,2,1]: D3→D2(8)+D2→D1(2) = 10dk arc.
  Alternatif [2,3,1]: D2→D3(8)+D3→D1(8) = 16dk arc.
  Flip eşiği: delay2 > 6dk → [3,2,1] pahalanır, [2,3,1] daha ucuz.
  ML gerçek tahmini yerine mock delay değerleri kullanılır.
""")
...
if reroute_found_at is not None:
    check("Rota değişim noktası tespit edildi",
          True,
          f"D2 hist_prob={reroute_found_at} seviyesinde rota değişti.")
    check("Gecikme → Maliyet dönüşümü doğrulanabilir",
          True,
          f"OR-Tools formülü: cost = (seyahat_dk + gecikme_dk) × 100")
else:
    check("Rota değişim noktası", False,
          "Test verisi bu aralıkta rotayı bükmedi — model bu senaryoda delay üretmedi.")
...
(will_miss=True → başa çekilmesi beklenir — soft penalty baskısı)
""")
...
ml_heavy = predict_route(HEAVY_STOPS)["stop_predictions"]
...
check("120 dk gecikmeli durak erken sıralanıyor (ilk 2 içinde)",
      heavy_stop_pos <= 1,
...
check("Penalty artışı will_miss durağını öne çekiyor",
      results_penalty[50_000] <= results_penalty[5_000],
      f"5k-penalty pozisyon={results_penalty[5_000]}, "
      f"50k-penalty pozisyon={results_penalty[50_000]}")
...
try:
    ...
    print(f"  \"{explanation}\"")
    check("Ollama dispatcher açıklaması üretildi", bool(explanation))

except Exception as e:
    print(f"  [{WARN}] Ollama çalışmıyor ({e}) — bu test atlandı.")
...
print("  │                             │ rota değişim eşiği tespit edildi │")
print("  │ Test-2A: Aşırı Gecikme      │ 120 dk → soft-penalty = başa çek │")
print("  │ Test-2B: Penalty Kalibre    │ 10× ceza → daha erken sıralama   │")
print("  │ Test-2C: Çakışan Pencereler │ Mesafe vs. gecikme dengesi        │")
print("  │ Test-3: Gürültü Stabilitesi │ ±5 dk hata → rota kararlılığı    │")
print("  └─────────────────────────────┴──────────────────────────────────┘")
